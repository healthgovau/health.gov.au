...
function health_adminimal_media_thumbnail($variables) {
  ...
  // Metadata.
  $metadata = '<div class="media-thumbnail__metadata">';
  $metadata .= '<div class="media-thumbnail__title">' . $element['#file']->filename . '</div>';

  $description = health_get_friendly_mime($element['#file']->filemime);
  $file_size = explode(' ', format_size($element['#file']->filesize));
  ...
  $description .= ' | ' . $formatted_filesize;
  // Images.
  if ($element['#file']->type == 'image') {
    if ($element['#file']->alt == '') {
      $element['#file']->alt = 'No alt text';
    }
    $description = $element['#file']->alt . '<br>' . $description;
  }
  // Documents.
  if ($element['#file']->type == 'document') {
    if ($element['#file']->field_resource_file_pages) {
      $description .= ' | ' . $element['#file']->field_resource_file_pages[LANGUAGE_NONE][0]['value'] . ' pages';
    }
  }
  $metadata .= '<div class="media-thumbnail__description">' . $description . '</div>';
  $metadata .= '</div>';

  // Arguments for the thumbnail link.
  $thumb = '<div class="media-thumbnail__image">' . $element['#children'] . '</div>' . $metadata;
...
function health_adminimal_file_link($variables) {
  $file = $variables['file'];
